...
The rule for what each SPM target may `import`. PureYAML is a monorepo from day one with many targets in one package, and the engine (`PureYAML`) and CLI (`pure-yaml`) are already two targets, so this contract applies now and governs every target added to the manifest.
...
2. Is the target an `executableTarget` / composition root (for example the `pure-yaml` CLI)? Allowed; the composition root wires the universe.
