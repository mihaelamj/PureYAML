...
let appleOnlyProducts: [Product] = [
    .singleTargetLibrary("PureYAMLUI"),
    .singleTargetLibrary("PureYAMLComponents"),
    // ...
]
#else
let appleOnlyProducts: [Product] = []
...
Same pattern for `appleOnlyTargets`. Core engine products (e.g. `PureYAML`) stay unconditional.
...
```swift
#if os(iOS)
return "PureYAML/iOS/\(version)"
#elseif os(macOS)
return "PureYAML/macOS/\(version)"
#elseif os(visionOS)
return "PureYAML/visionOS/\(version)"
#else
return "PureYAML/unknown/\(version)"  // Linux falls into "unknown"
#endif
```
...
return "PureYAML/visionOS/\(version)"
#elseif os(Linux)
return "PureYAML/Linux/\(version)"
#else
return "PureYAML/unknown/\(version)"  // truly unknown future platform
#endif
```
