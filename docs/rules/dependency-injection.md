...
Canonical dependency-injection principles for PureYAML and the wider PureYAML codebase: no singletons, constructor injection, protocol seams.
...
If and when PureYAML grows into multiple SPM packages:
...
If and when PureYAML is split into multiple packages, each package should lift out of the repo without modification:
...
If PureYAML ships multiple binaries, the **executable target name** stays normal (no `Impl` suffix). The binary's external identity (filename on disk, `which` output) stays clean.
