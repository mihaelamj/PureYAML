...
The operational namespacing discipline for PureYAML: folder mirroring, file naming, anchor placement, one-type-per-file, reserved names, namespace-vs-module collisions.
...
This applies to every Swift project. The PureYAML-style root + sub-namespace pattern below is one specific instance of this discipline.
...
```text
PureYAML.Parser.Model.Token          // from PureYAMLCore
PureYAML.Generator.Command           // from PureYAMLCLI
PureYAML.Parser.Scanning.Scanner     // from PureYAMLTools
```
...
## Root + sub-namespace pattern (PureYAML style)
...
```swift
/// Root namespace for all PureYAML types across all modules
...
- Root namespace = one file (e.g. `PureYAML.swift`)
...
extension PureYAML.Parser.Model {
...
extension PureYAML.Parser.Model.Token: CustomStringConvertible {
...
- Type lives under the correct semantic path, e.g.:
  - `PureYAML.Model`
  - `PureYAML.Parser.Model`
  - `PureYAML.Parser.Metadata`
  - `PureYAML.Generator`
- Conformances (`Codable`, `Sendable`, `CustomStringConvertible`, etc.) MAY be separate extensions

### File layout for PureYAML-style namespacing
...
```text
PureYAML/
    PureYAML.swift                       // namespace anchor: public enum PureYAML {}
    Parser/Model/Token.swift             // extension PureYAML.Parser.Model { public enum Token {} }
    Parser/Scanning/Scanner.swift        // extension PureYAML.Parser.Scanning { public struct Scanner {} }
    Generator/Writer.swift               // extension PureYAML.Generator { public struct Writer {} }
```
...
```text
PureYAML.swift                              // namespace anchor: public enum PureYAML {}
PureYAML.Parser.Model.Token.swift           // extension PureYAML.Parser.Model { public enum Token {} }
PureYAML.Parser.Metadata.Header.swift       // extension PureYAML.Parser.Metadata { public struct Header {} }
```
...
- `PureYAML.swift` declaring `public enum PureYAML { /* sub-namespaces */ }`
...
Concrete example, a `PureYAML` namespace umbrella:

```text
Sources/PureYAML/
├── PureYAML.swift          // namespace anchor: extension PureYAML { ... }      <-- AT ROOT, not Core/PureYAML.swift
├── Client/                // PureYAMLClient SPM target
├── Core/                  // PureYAMLCore SPM target (path includes ../PureYAML.swift)
│   ├── Protocol/
│   ├── Server/
│   └── Transport/
├── SharedTools/           // PureYAMLSharedTools SPM target
└── Support/               // PureYAMLSupport SPM target
```

The PureYAMLCore target's `Package.swift` entry uses `path: "Sources/PureYAML"` with `exclude: ["Client", "SharedTools", "Support"]` so it picks up the anchor file plus the `Core/` subtree. Every sibling target (Client, SharedTools, Support) depends on PureYAMLCore in production, so the `PureYAML` namespace anchor reaches them through that dep.

Why this matters:

- Reading the file listing immediately tells you "PureYAML is the umbrella here": the anchor sits visually at the top.
- Moving an anchor into `Core/PureYAML.swift` (or any other sub-target folder) hides the umbrella relationship behind one extra `cd` and one extra mental indirection.
